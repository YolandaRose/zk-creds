...
/// U.S. State Department passport signing key (real ePassport SOD). Use only if the dump still
/// contains a genuine signature from that PKI. See
/// <https://github.com/AndyQ/NFCPassportReader/tree/master/scripts>.
const USA_PUBKEY_PEM: &str = "\
-----BEGIN PUBLIC KEY-----
...
pub fn load_usa_pubkey() -> IssuerPubkey {
    let pubkey = RsaPublicKey::from_public_key_pem(USA_PUBKEY_PEM).unwrap();
    IssuerPubkey(pubkey)
}
